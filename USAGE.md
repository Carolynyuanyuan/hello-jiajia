...
- **智能时间范围爬取**：自动爬取上一周（最近7天）的所有文章，不限数量
- **按发布时间过滤**：系统智能判断文章发布时间，只保存指定时间范围内的文章
- **自动停止**：当连续遇到超出时间范围的文章时自动停止爬取，节省资源
- **每周自动执行**：每周一早上9:00自动爬取并生成周摘要
...
A: 系统已设置随机延迟和合理的爬取频率。每周只爬取一次，自动获取上周的所有文章，风险较低。但仍建议：
- 不要频繁手动测试
- 时间范围设置合理（建议7-14天）
- 如遇访问限制，等待几小时后再试

**工作原理**：系统会智能判断文章发布时间，只爬取指定时间范围内的文章，当连续遇到超出时间范围的文章时自动停止。
